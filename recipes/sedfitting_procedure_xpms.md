...
**IDL>**
	...
	if nwav ge 12 then shift = 2 else shift = 0
	magfromdata,data,0+shift,j,nwav=nwav,mk=mk
	magfromdata,data,1+shift,h,nwav=nwav,mk=mk
	magfromdata,data,2+shift,k,nwav=nwav,mk=mk
...
Use IDL to make a new fitter data file and SAOImage ds9 regionfile containing only the well-fit sources. Create the `results_xpms` directory contining model fitting results as IDL save files.
  ...
  **IDL>**
  	...
  	fitinfo2data,parfile_good,'data_xir_noH','data_xpms_good'
